...
* Im Linear mixed Model dürfen Regressionsparameter variieren.
...
p4 <- ggplot(df_random_intercept_slope, aes(x = x, y = y, color = group)) +
  ...
  annotate("text", x = -1, y = coef(lm3)["(Intercept)"] + coef(lm3)["group2"], label = expression(beta["02"]), parse = TRUE, size = 5) +
  annotate("text", x = -1, y = coef(lm3)["(Intercept)"] + coef(lm3)["group3"] -1, label = expression(beta["03"]), parse = TRUE, size = 5) +
